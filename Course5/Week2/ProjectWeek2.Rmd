...
There are `r na_num` missing values in the dataset (about `r round(na_pc)` %). It's important. So, we can't ignore them.

**Devise a strategy for filling in all of the missing values in the dataset. The strategy does not need to be sophisticated. For example, you could use the mean/median for that day, or the mean for that 5-minute interval, etc.**

My strategy : Replace the missing values by the mean of the considered interval.
...
First, set the week name in English
```{r}
